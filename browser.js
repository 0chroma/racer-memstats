...
(function() {
  var Collection, Document, FieldSizes, FieldValue, Format, Model, _, sizeof;
  ...
  Model = {
    getFieldSizes: function(model) {
      return Model._mapCollections(model, function(collection) {
        return Collection.getFieldSizes(collection);
      }).reduce(FieldSizes.merge, {});
    },
    getCollectionSizes: function(model) {
      return _.zipObject(Model._mapCollections(model, function(collection, name) {
        return [name, Collection.getSize(collection)];
      }));
    },
    getDocumentSizes: function(model) {
      return _.merge.apply(null, Model._mapCollections(model, Collection.getDocumentSizes));
    },
    getSize: function(model) {
      return _.sum(_.values(Model.getCollectionSizes(model)));
    },
    _mapCollections: function(model, fn) {
      return Model._getCollectionList(model).map(function(collectionName) {
        return fn(model.get(collectionName), collectionName);
      });
    },
    _getCollectionList: function(model) {
      var collections, isChar;
  ...
  Collection = {
    getDocumentSizes: function(collection) {
      return _.mapValues(collection, Document.getSize);
    },
    getFieldSizes: function(collection) {
      var docs;
      docs = _.values(collection);
      return docs.map(Document.getFieldSizes).reduce(FieldSizes.merge, {});
    },
    getSize: function(collection) {
      var docSizes;
      docSizes = Collection.getDocumentSizes(collection);
      return _.sum(_.values(docSizes));
    }
  };
  ...
  Document = {
    getSize: function(doc) {
      ...
      return _.sum(_.values(sizes));
    },
    getFieldSizes: function(doc) {
      var fieldSizes;
      fieldSizes = _.pairs(doc).map(function(val) {
        return [val[0], FieldValue.getSize(val)];
      });
      return _.zipObject(fieldSizes);
    }
  };

  FieldSizes = {
    merge: function(fieldSizesA, fieldSizesB) {
      return _.merge(fieldSizesA, fieldSizesB, function(a, b) {
        return (a || 0) + (b || 0);
      });
    }
  };
  ...
  FieldValue = {
    getSize: function(fieldvalue) {
      return sizeof(fieldvalue[1]);
    }
  };

  Format = {
    sortedTable: function(resultObj) {
      return console.table(_(resultObj).pairs().sortBy(function(n) {
        return n[1];
      }).value());
    }
  };

  module.exports = window.racerms = {
    Model: Model,
